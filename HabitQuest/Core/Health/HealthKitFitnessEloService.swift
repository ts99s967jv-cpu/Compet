...
final class HealthKitFitnessEloService {
  ...
  private func fetchDailyDiscreteAverage(
    ...
    let dailyValues = daily.map { $0 ?? avg } // fill gaps deterministically for consistency calc
  ...
  private func fetchLatestQuantity(
    ...
    let daysPresent = 1
    return (metric, .init(value: v, daysPresent: daysPresent, windowDays: windowDays, dailyValues: [v]))
  }
  ...
  private func fetchDailySleepHours(
    ...
    let endDay = Calendar.current.startOfDay(for: end)
    while cursor <= endDay {
      let seconds = byDaySeconds[cursor] ?? 0
