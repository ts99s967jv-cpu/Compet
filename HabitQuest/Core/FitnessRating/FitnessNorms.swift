...
/// v2: age-normalized population norms for percentile conversion.
///
/// Spec requirement:
/// - No fixed "healthy ranges"
/// - Convert values to age-normalized percentiles
/// - Initially use static tables; later replace with live distributions
///
/// Implementation approach:
/// - Store Normal(μ, σ) parameters by age cohort.
/// - Convert via normal CDF to percentile in [0, 1].
/// - For metrics where "lower is better" (e.g. resting HR), invert percentile.
enum FitnessNorms {
  ...
  static func percentile(
    ...
    guard let params = paramsByMetricAndCohort[metric]?[cohort.label] else {
      // Fallback if a cohort table is missing.
      return 0.5
    }
    ...
    if metric == .restingHeartRate || metric == .respiratoryRate {
      // Lower is generally better; invert.
      return 1.0 - clamped
    }
    return clamped
  }

  // MARK: Static initial tables (coarse, adjustable)
  //
  // Notes:
  // - These are intentionally "reasonable starting points" rather than medical claims.
  // - They provide deterministic percentiles until live cohort distributions exist.
  ...
  private static func erfApprox(_ x: Double) -> Double {
    // Use platform erf when available; fall back to a deterministic approximation.
    #if canImport(Darwin)
    return Darwin.erf(x)
    #elseif canImport(Glibc)
    return Glibc.erf(x)
